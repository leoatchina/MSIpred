...
def reduce_maf_df(tagged_maf_file):
    ...
    for chunk in maf_reader:
        chunks.append(chunk)
    reduced_df = concat(chunks, axis=0)
    return reduced_df


def count_vt_all(reduced_df, exome_size):  # this data is from a reference article
    '''
    ...
    Input:
            reduced_df: a dataframe generated by function reduce_maf_df
    ...
    grouped_df = reduced_df[['Variant_Type', 'In_repeats']].groupby(reduced_df['Tumor_Sample_Barcode'])     # a groupby object: index is tumor barcode, values is a series holding Variant_Type for that group(index)
    for tumor, group in grouped_df:
        ...
        inner_list = [
            tumor,
            vt_list.count('SNP')/exome_size,
            vt_list.count('INS')/exome_size,
            vt_list.count('DEL')/exome_size,
            in_repeats_vt.count('SNP')/exome_size,
            in_repeats_vt.count('INS')/exome_size,
            in_repeats_vt.count('DEL')/exome_size,
        ]
...
def count_vc_all(reduced_df, exome_size):
    '''
    ...
    grouped_df = reduced_df[['Variant_Classification', 'In_repeats']].groupby(reduced_df['Tumor_Sample_Barcode'])
    for tumor, group in grouped_df:
        inner_list = [tumor]
        ...
        for item in variant_class:
            counts = vc_list.count(item)/exome_size
...
class Tagged_Maf(object):
    ...
    def make_feature_table(self, exome_size):
        ...
        reduced_df = reduce_maf_df(tagged_maf_file)
        vt_feature = count_vt_all(reduced_df, exome_size)
        vc_feature = count_vc_all(reduced_df, exome_size)
